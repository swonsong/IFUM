...
arg parsing(csv_dir, out_FASTA_dir, out_PDB_dir) -> read csv files and extract sequence, metadata(split, dG) by each row -> run ESMFold prediction on the sequence -> save PDB files to out_dir
"""
import torch
...
def get_args():
    ...
    parser.add_argument('--out_FASTA_dir', type=str, required=True, help='Output directory for .fasta files')
    parser.add_argument('--out_PDB_dir', type=str, required=True, help='Output directory for .pdb files')
...
def process_csv_files(csv_dir, out_FASTA_dir):
    csv_files = glob(os.path.join(csv_dir, "*.csv"))
    metadata = {}
    for csv_file in csv_files:
        name = os.path.splitext(os.path.basename(csv_file))[0]
        fasta_path = os.path.join(out_FASTA_dir, f"{name}.fasta")
        ...
        with open(fasta_path, 'w') as f:
            for idx, row in df.iterrows():
                ...
                # Clean name: remove common extensions from sequence header if they exist (e.g. .cif, .pdb)
                clean_p_name = os.path.splitext(p_name)[0]
                
                # Write FASTA format: name (row1), sequence (row2)
                f.write(f">{clean_p_name}\n{seq}\n")
                
                # Save metadata: split, dG (deltaG)
                split = row.get('split', 'train')
                dG = row.get('deltaG', 0.0)
                metadata[clean_p_name] = {
                    'split': split,
                    'deltaG': float(dG)
                }
                
        logger.info(f"Wrote FASTA for {name} to {fasta_path}")
        
    # Save the accumulated metadata dictionary to metadata.json in out_FASTA_dir
    metadata_path = os.path.join(out_FASTA_dir, "metadata.json")
...
def main():
    ...
    process_csv_files(args.csv_dir, args.out_FASTA_dir)
    
    # 2. Run ESMFold prediction on each generated FASTA file
    logger.info("--- Step 2: Running ESMFold prediction on FASTA files ---")
    fasta_files = glob(os.path.join(args.out_FASTA_dir, "*.fasta"))
    if not fasta_files:
        logger.warning(f"No FASTA files found in {args.out_FASTA_dir}. Pipeline exiting.")
        return
