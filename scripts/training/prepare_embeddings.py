...
def main():
    ...
    # --- Process pdb/cif files ---
    files = glob(os.path.join(args.pdb_dir, "*.cif")) + glob(os.path.join(args.pdb_dir, "*.pdb"))
    print(f"Found {len(files)} pdb/cif files.")

    with torch.no_grad():
        for pdb_path in tqdm(files):
            try:
