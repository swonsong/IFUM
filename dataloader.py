...
def run_esmfold(input_csv, out_dir, device, num_recycles=None, max_tokens_per_batch=1024, chunk_size=None):
    ...
    logger.info(f"Reading sequences from input DataFrame")
    ...
    for headers, sequences in batched_sequences:
        ...
        for header, seq, pdb_string in zip(headers, sequences, pdbs):
            ...
            try:
                ...
                logger.error(f"Failed to write PDB for {header}: {e}")
...
def main():
    ...
    if input_csv.empty:
        logger.error("No valid sequences found. Exiting.")
        sys.exit(1)
